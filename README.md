...
4. Give them to the app as `SUPABASE_URL` and `SUPABASE_ANON_KEY`. The app reads `window.SUPABASE_URL` and `window.SUPABASE_ANON_KEY` from `config.js`, which is gitignored and generated by `npm run build` (`scripts/write-config.mjs`):
   - **Vercel**: add both as Environment Variables for *Production* **and** *Preview*. `vercel.json` sets the build command to `npm run build`, so every deployment, branch previews included, writes its own `config.js`. The build log prints `config.js written for <host>`; a warning there means a variable is missing for that environment.
   - **Locally**: copy `.env.example` to `.env`, fill it in and run `npm run build` once (or copy `config.template.js` to `config.js` and fill that in by hand).
